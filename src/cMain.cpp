...
cMain::cMain() : wxFrame(nullptr, wxID_ANY, "MCBE DLL Injector", wxDefaultPosition, wxSize(297.5, 162.5), wxMINIMIZE_BOX | wxSYSTEM_MENU | wxCAPTION | wxCLOSE_BOX | wxCLIP_CHILDREN) {
    ...
    taskBarControl.SetIcon(icon, "Double-click to show the DLL Injector");

    notification = new wxNotificationMessage("MCBE DLL Injector", "The DLL Injector is now hidden in your system tray", this, 0);
    notification->UseTaskBarIcon(&taskBarControl);
    openDialog = new wxFileDialog(this, "Select the .dll file", working_dir, "*.dll" ,"Dynamic link library (*.dll)|*.dll", wxFD_OPEN);
}
